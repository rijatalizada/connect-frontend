...
const Profile = () => {
  ...
  const [user, loading, error] = useFetch(getUser + userId);
  ...
  // if(error){
  //   return <div className="my-56 text-center text-[2rem]">An error occured while fetching data, try later or contact with us!</div>
  // }
      ...
      <main className="container my-32 mx-auto ">
        <div className="mb-10 flex w-full justify-between">
          ...
          <div>
            <Link
              ...
              className="text-color-primary border-[#1d84b5] border-solid border-2 p-2 rounded-md duration-[0.4s] hover:text-color-secondary hover:bg-color-primary "
            >
              Update
            </Link>
            <Link to={"/searchUsers"} className="text-color-primary mx-3 border-[#1d84b5] border-solid border-2 p-2 rounded-md duration-[0.4s] hover:text-color-secondary hover:bg-color-primary ">Search Other users</Link>
          </div>
        </div>
        <div className="grid lg:grid-cols-[58%,42%] gap-6 border-b-2 border-[#1d84b5] pb-10">
          <div id="bio" className="flex justify-center items-center">
            <div className="mt-10">
              <p className="text-[1rem] text-[#00000075]">My Bio</p>
